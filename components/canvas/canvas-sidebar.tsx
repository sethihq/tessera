...
export function CanvasSidebar({ 
  ...
  const getAssetIcon = (type: Asset['type']) => {
    switch (type) {
      case 'sprite-sheet': return <GridIcon className="w-4 h-4 text-purple-500" />
      case 'parallax': return <GridIcon className="w-4 h-4 text-blue-500" />
      case 'style-ref': return <GridIcon className="w-4 h-4 text-green-500" />
      case 'tileset': return <GridIcon className="w-4 h-4 text-orange-500" />
    }
  }

  const getStatusBadge = (status: Asset['status']) => {
    switch (status) {
      case 'completed': return <Badge variant="outline" className="text-xs bg-green-50 text-green-700">Ready</Badge>
      case 'generating': return <Badge variant="outline" className="text-xs bg-blue-50 text-blue-700">Processing</Badge>
      case 'draft': return <Badge variant="outline" className="text-xs bg-gray-50 text-gray-700">Draft</Badge>
    }
  }
  ...
  if (isCollapsed) {
    return (
      <div className={`${side === 'left' ? 'border-r' : 'border-l'} border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900`}>
        <div className="w-12 h-full flex flex-col items-center py-4 space-y-2">
          ...
          >
            {side === 'left' ? <ChevronRightIcon className="w-4 h-4" /> : <ChevronRightIcon className="w-4 h-4 rotate-180" />}
          </Button>
        </div>
      </div>
    )
      ...
      >
        ...
        <div className="p-4 border-t border-neutral-200 dark:border-neutral-800">
          <div className="space-y-2">
            <Button className="w-full justify-start h-8 text-sm bg-blue-600 hover:bg-blue-700">
        ...
        <div className="p-4 space-y-4">
          {selectedAsset ? (
            <>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <Button className="w-full h-7 text-sm">
                    Export Asset
                  </Button>
                </CardContent>
              </Card>
            ...
            <div className="text-center py-8 text-neutral-500 dark:text-neutral-400">
              <GridIcon className="w-8 h-8 mx-auto mb-3 opacity-50" />
